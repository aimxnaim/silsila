...
import type { Signal } from './insights.ts';
...
import { criticalRoles, meanSpan, spans } from './structure.ts';
 ...
 * support.
 */
function progressionSignal(p: Progression | null): Signal | null {
...
function spanSignal(seat: PersonSeat | null, name: string): Signal | null {
...
function successionSignal(seat: PersonSeat | null, name: string): Signal | null {
...
export function personAnalysis(
  ...
  const found = [
    successionSignal(seat, person.name),
    spanSignal(seat, person.name),
    stagnationSignal(model, personId, held.length, inSeat, yearsService),
